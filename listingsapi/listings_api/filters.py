...
VIVAREAL_MAX_SALE_PRICE = 700000
VIVAREAL_MAX_RENTAL_PRICE = 4000
...
def listings_zap_filter(listings):
    ...
    zap_listings = [x for x in zap_listings if (int(x.pricingInfos.price) >= 3500
                    ...
                    or (((x.isInsideZapGroupArea and
                        int(x.pricingInfos.price) >= ZAP_MIN_SALE_PRICE * 0.9) or int(x.pricingInfos.price) >= ZAP_MIN_SALE_PRICE)
                        and x.pricingInfos.businessType == "SALE"
                        and (x.usableAreas == 0 or x.squareMeterPrice > ZAP_MIN_RENTAL_PRICE))]
...
def listings_vivareal_filter(listings):
    zap_listings = remove_listings_without_location_filter(listings)
    vivareal = [x for x in zap_listings if (((x.isInsideZapGroupArea and int(x.pricingInfos.price) <= VIVAREAL_MAX_RENTAL_PRICE * 1.5)
                                            or int(x.pricingInfos.price) <= VIVAREAL_MAX_RENTAL_PRICE)
                                            and x.pricingInfos.businessType == "RENTAL"
                ...
                or (int(x.pricingInfos.price) <= VIVAREAL_MAX_SALE_PRICE
                    and x.pricingInfos.businessType == "SALE")]
